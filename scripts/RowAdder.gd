# RowAdder.gd
# Suma una fila origen a una fila destino y actualiza los tiles visibles.

extends RefCounted
class_name RowAdder


static func add_rows(matrix_container: GridContainer, n: int, source_row: int, target_row: int, row_colors: Array) -> void:
	...
	if total_children == 0 or total_children % visual_cols != 0:
		push_warning("RowAdder.add_rows: child count no cuadra con filas completas.")
		return

	var num_rows := total_children / visual_cols
	if source_row < 0 or source_row >= num_rows:
		push_warning("RowAdder.add_rows: fila origen fuera de rango.")
		return
	if target_row < 0 or target_row >= num_rows:
		push_warning("RowAdder.add_rows: fila destino fuera de rango.")
		return
	...
	for logical_col in range(logical_total_cols):
		...
		var new_val := dst_val.add(src_val)
